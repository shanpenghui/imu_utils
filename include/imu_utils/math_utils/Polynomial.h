...
class Polynomial
{
  ...
  private:
    ...
    // _y = f(x)
    bool FindRoots(const double y, const eigen_utils::Vector& polynomial_in,
                   eigen_utils::Vector& real, eigen_utils::Vector& imaginary);

    void FindLinearPolynomialRoots(const eigen_utils::Vector& Polynomial,
    ...
    void BuildCompanionMatrix(const eigen_utils::Vector& Polynomial,
                              eigen_utils::Matrix* companion_matrix_ptr);
...
class PolynomialFit : public Polynomial
{
  public:
    ...
    void loadSamples(const eigen_utils::Vector& x,
                     const eigen_utils::Vector& y);
